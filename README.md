...
The separate “See Inside” reader appears beneath the Christmas Story product and “Coming soon”; the original cover image stays in place and remains non-clickable. Clicking the reader's cover opens through the actual front matter to the title spread. Clicking the left/right page or using the arrow controls turns backward/forward; Escape, Close book, or a click outside closes it to the front cover. Narrow layouts display one page at a time and preserve native pinch zoom. The reader includes accessible page text, supports keyboard arrows, honors reduced motion, preloads nearby pages rather than downloading the whole book at page load, and allows retry after a page-image loading failure.

Reader content is the complete approved Canonical 126 master: 55 source-rendered WebP images, including all 52 printed interior pages, plus one plain inside-back binding leaf for correct cover parity. The page images are 1600 × 1236; text pages use lossless encoding. No PDF, native InDesign files, print package, or private provenance is published. Source rendering and visual checks are recorded locally in `output/book-reader-assets/_private/`; the public reading-order manifest is `dist/assets/book/126/manifest.json`. The builder verifies the complete sequence and only publishes explicitly selected assets.
